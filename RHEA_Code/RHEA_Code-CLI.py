# -*- coding: utf-8 -*-
# RHEA_Code-CLI.py
# RHEA Code CLI – Full Native RHEA-UCM Agentic Coding Assistant (v4.2 Hardened Monolith)
...
class GlyphParser:
    ...
    def parse(self, text: str) -> Dict[str, Any]:
        ...
        # ---------------------- Trace profiler ----------------------
            ...
            role = "trace_off"
        elif cmd == "trace clear failures":
            role = "trace_clear"
        elif cmd == "trace clear":
            role = "trace_clear"
        elif cmd.startswith("trace failures"):
            role = "trace_failures"
        elif cmd.startswith("trace list"):
            role = "trace_list"
        elif cmd.startswith("trace open"):
            role = "trace_open"
...
class RHEACodeCLI:
    ...
    def _banner(self) -> None:
        self._out("=" * 88)
        self._out("RHEA Code CLI v4.2 Hardened initialized — Full Entropy-Trust Control Active")
    ...
    def _normalize_multiline_payload(self, payload: str, old_text: str, append: bool) -> str:
        if not payload:
            return old_text if append else payload
    ...
    def _merge_edit_content(self, old_text: str, content: Optional[str], append: bool, content_mode: Optional[str]) -> str:
        if content_mode in {"pastefile", "pasteappend"}:
            ...
            return self._normalize_multiline_payload(payload, old_text, do_append)

        payload = content if content is not None else "# RHEA edit\n"
        if append:
            return self._normalize_multiline_payload(payload, old_text, True)
        return payload

    def _finalize_successful_edit(
        self,
        *,
        result: str,
        checkpoint_note: str,
        ...
        return f"{prefix}{result} {success_message}"
    ...
    def tool_edit(
        ...
        **kwargs: Any,
        ...
        try:
            new_text = self._merge_edit_content(old_text, content, append, content_mode)
        except ValueError as e:
            return str(e)
        ...
        action = "Appended pasted content to" if content_mode == "pasteappend" else (
            "Wrote pasted content to" if content_mode == "pastefile" else (
                "Appended to" if append else "Wrote"
            )
        )
        commit_note = self._maybe_commit_after_edit(op="edit", file=file)
        return self._finalize_successful_edit(
            result=result,
            checkpoint_note=checkpoint_note,
            commit_note=commit_note,
            success_message=f"{action} {target}",
        )
    ...
    def tool_replace(
        ...
        **kwargs: Any,
        ...
        if result.startswith("Change applied"):
            commit_note = self._maybe_commit_after_edit(op="replace", file=file)
            return self._finalize_successful_edit(
                result=result,
                checkpoint_note=checkpoint_note,
                commit_note=commit_note,
                success_message=f"Replaced {replaced} occurrence(s) in {target}",
            )
        return result
    ...
    def _insert_relative(
        ...
        if result.startswith("Change applied"):
            ...
            return self._finalize_successful_edit(
                ...
                checkpoint_note=checkpoint_note,
                commit_note=commit_note,
                success_message=f'{action} anchor in {target.name}: "{anchor}"',
            )
        return result
    ...
    def tool_prepend(
        ...
        **kwargs: Any,
        ...
        result = self._apply_change_with_preview(target, updated)
        if result.startswith("Change applied"):
            commit_note = self._maybe_commit_after_edit(op="prepend", file=file)
            return self._finalize_successful_edit(
                result=result,
                checkpoint_note=checkpoint_note,
                commit_note=commit_note,
                success_message=f"Prepended content to {target}",
            )
        return result
        ...
        **kwargs: Any,
        ...
        if result.startswith("Change applied"):
            ...
            if commit_note:
                prefix_parts.append(commit_note)
            prefix = ("\n".join(prefix_parts) + "\n") if prefix_parts else ""
            return f"{prefix}{result} Replaced line {line_no} in {target}"
        return result
        ...
        **kwargs: Any,
        ...
        if result.startswith("Change applied"):
            ...
            if commit_note:
                prefix_parts.append(commit_note)
            prefix = ("\n".join(prefix_parts) + "\n") if prefix_parts else ""
            return f"{prefix}{result} Replaced lines {start_line}:{end_line} in {target}"
        return result
    ...
    def _char_edit(
        ...
        if result.startswith("Change applied"):
            ...
            return f"{prefix}{result} {mode_word} char at line {line_no}, char {char_pos} in {target}"
        return result
        ...
        **kwargs: Any,
        ...
        if result.startswith("Change applied"):
            ...
            if commit_note:
                prefix_parts.append(commit_note)
            prefix = ("\n".join(prefix_parts) + "\n") if prefix_parts else ""
            return f"{prefix}{result} Replaced word in {target}"
        return result
        ...
        **kwargs: Any,
        ...
        if result.startswith("Change applied"):
            ...
            if commit_note:
                prefix_parts.append(commit_note)
            prefix = ("\n".join(prefix_parts) + "\n") if prefix_parts else ""
            return f"{prefix}{result} Replaced current selection in {target}"
        return result
    ...
    def _maybe_commit_after_edit(
        ...
        choice = input("Commit this change now? [y/N]: ").strip().lower()
        ...
        if file:
            stage_result = git_stage_file(self.cwd, file)
            if "[exit=0]" not in stage_result:
                return f"[commit skipped] Could not stage file: {file}\n{stage_result}"
        ...
        msg = build_checkpoint_message(
            ...
            target=target,
            lines=lines,
            note="manual commit after successful edit",
        )
        commit_result = git_commit(self.cwd, msg)
        if "[exit=0]" in commit_result:
            return f"[commit created] {msg}"
        lower = commit_result.lower()
        if "nothing to commit" in lower or "nothing added to commit" in lower:
            return "[commit skipped] No changes to commit."
    ...
    def tool_trace_list(self, limit: int = 20, **kwargs: Any) -> str:
        files = self.profiler.list_trace_files(failures_only=False, limit=limit)
        if not files:
    ...
    def tool_trace_failures(self, limit: int = 20, **kwargs: Any) -> str:
        files = self.profiler.list_trace_files(failures_only=True, limit=limit)
        if not files:
    ...
    def tool_trace_open(self, filename: Optional[str] = None, **kwargs: Any) -> str:
        ...
        safe_filename = self._validate_user_token(filename, "trace filename")
        return self.profiler.read_trace_file(safe_filename)
    ...
    def tool_trace_clear(self, failures_only: bool = False, **kwargs: Any) -> str:
        ...
        return self.profiler.clear_trace_files(failures_only=failures_only)
    ...
    def tool_run(
        ...
        **kwargs: Any,
        ...
        safe_cmd = self._validate_shell_command(cmd)
        if not safe_cmd:
    ...
    def tool_help(self, **kwargs: Any) -> str:
        return (
            ...
            "  read def RHEA_Code-CLI.py tool_help\n\n"
            ...
            "  select lines RHEA_Code-CLI.py 10:20\n"
            "  read selection\n"
            "  replace selection pastefile\n"
            "  clear selection\n\n"

            "VCS (Git):\n"
            "  git mode off|manual|auto\n"
            "  git status\n"
            "  git diff\n"
            "  git stage RHEA_Code-CLI.py\n"
            "  git commit \"message\"\n\n"
    ...
    def _extract_args(self, role: str, raw_cmd: str) -> dict:
        ...
        if role == "trace_off":
            return {}
        if role == "trace_list":
            return self._extract_trace_list_args(stripped)
        if role == "trace_failures":
            return self._extract_trace_failures_args(stripped)
        if role == "trace_open":
            return self._extract_trace_open_args(stripped)
        if role == "trace_clear":
            return self._extract_trace_clear_args(stripped)

        if role == "run":
            return {"cmd": self._extract_run_command(stripped), "check": True}
        if role == "git":
    ...
    def _extract_trace_list_args(self, text: str) -> dict:
        tokens = self._safe_split(text)
        limit = 20
        if len(tokens) >= 3:
            try:
    ...
    def _extract_trace_failures_args(self, text: str) -> dict:
        tokens = self._safe_split(text)
        limit = 20
        if len(tokens) >= 3:
            try:
    ...
    def _extract_trace_open_args(self, text: str) -> dict:
        tokens = self._safe_split(text)
        filename = tokens[2] if len(tokens) >= 3 else None
        return {"filename": filename}

    def _extract_trace_clear_args(self, text: str) -> dict:
